...
// { from, to, via, year, month, day, hour, minutes, boolean airplane, boolean bus, boolean express, mode = shuppatsuetc, transfer_mode
    function translateCommand(command){
        ...
        if ( command.from && command.to ){
            ...
            tmp = "";
            if ( command.year && parseInt(command.year, 10) >= 0 ) {
                tmp = command.year;
            }
            else{
            ...
            if ( command.month && parseInt(command.month, 10) > 0 && parseInt(command.month, 10) < 13){
                if(parseInt(command.month, 10) < 10){
                    tmp += "0";
                }
                tmp += command.month;
                month = command.month;
            ...
            if (command.day && parseInt(command.day, 10) > 0 && parseInt(command.day, 10) <  new Date(year,month,0).getDate()){
            ...
            if ( command.hour && parseInt(command.hour, 10) > 0 && parseInt(command.hour, 10) < 25 ){
                returned["Dhh"] = command.hour;
            }

            returned["Dmn2"] = date.getMinutes() % 10;
            returned["Dmn1"] = (date.getMinutes() - returned["Dmn2"])/10;
            if( command.minutes && parseInt(command.minutes, 10) >= 0 && parseInt(command.minutes, 10) < 60){
                returned["Dmn2"] = parseInt(command.minutes, 10) % 10;
                returned["Dmn1"] = (parseInt(command.minutes, 10) - returned["Dmn2"]) / 10;
            }
